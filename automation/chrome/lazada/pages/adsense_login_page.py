...
from automation.utils import wait_element, take_element_screenshot, slide_element, check_element_displayed
...
class LazadaAdsensePage():
    ...
    def change_lang(self, lang):
        if check_element_displayed(self.driver, self.change_lang_btn, -20):
            wait_element(self.driver, self.change_lang_btn).click()
            if lang == Languages.EN:
                wait_element(self.driver, self.lang_en_btn).click()
            elif lang == Languages.VI:
                wait_element(self.driver, self.lang_vi_btn).click()

    def login_w_type(self, login_type):
        if login_type == LoginTypes.ACCOUNT:
            if check_element_displayed(self.driver, self.login_w_account_btn):
                login_acc = wait_element(self.driver, self.login_w_account_btn)
                print(f"Found LOGIN TYPE: {login_acc}")
                login_acc.click()
            wait_element(self.driver, self.user_name_txt).send_keys('LAZ_ACC')
            wait_element(self.driver, self.password_txt).send_keys('LAZ_PASS')
            time.sleep(1)

            if check_element_displayed(self.driver, self.login_btn):
                wait_element(self.driver, self.login_btn).click()
            elif check_element_displayed(self.driver, self.slide_btn):
                slide_element(self.driver, self.slide_btn, 500, 0)

        elif login_type == LoginTypes.QR:
            if check_element_displayed(self.driver, self.login_w_qr_btn):
                login_qr = wait_element(self.driver, self.login_w_qr_btn)
                print(f"Found LOGIN TYPE: {login_qr}")
                login_qr.click()
            qr = wait_element(self.driver, self.qr_elm)
            take_element_screenshot(self.driver, qr, filename='qr.png')
            while not check_element_displayed(self.driver, self.affiliate_link_converter):
                count_down = 300
                while count_down > 0:
                    time.sleep(1)
                    count_down -= 1
                    if count_down % 60 == 0:
                        if check_element_displayed(self.driver, self.affiliate_link_converter):
                            break
                        else:
                            if check_element_displayed(self.driver, self.qr_lbl):
                                wait_element(self.driver, self.qr_lbl).click()
                if check_element_displayed(self.driver, self.affiliate_link_converter):
                    break
                else:
                    if check_element_displayed(self.driver, self.refresh_expired_qr_btn):
                        wait_element(self.driver, self.refresh_expired_qr_btn).click()
                        take_element_screenshot(self.driver, qr, filename='qr.png')

        # Login successful
        affiliate_converter_func = check_element_displayed(self.driver, self.affiliate_link_converter)
